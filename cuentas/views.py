...
from cuentas.forms import NuestroFormularioDeRegistro, NuestroEditarPerfilFormulario
...
def editar_perfil(request):
    ...
    if request.method == 'POST':
        formulario= NuestroEditarPerfilFormulario(request.POST, request.FILES, instance= request.user)
        if formulario.is_valid():
            ...
            formulario.save()
            return redirect("perfil_base")  
    else:
        formulario = NuestroEditarPerfilFormulario(initial= {'link': info_extra.link, 'avatar':info_extra.avatar},instance= request.user)
...
def perfil_base(request):
    return render(request, 'cuentas/perfil_base.html')